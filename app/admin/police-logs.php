                                        ...
                                        switch ($filter) {
                                            case 'today':
                                                $sql_select = "SELECT DISTINCT 
                                                        ...
                                                        WHERE 
                                                            queue_details.service = 'Police' AND queue_details.`status` = 1 
                                            ...
                                            case '7days':
                                                $sql_select = "SELECT DISTINCT 
                                                        ...
                                                        LEFT JOIN 
                                                            feedback ON client.client_id = feedback.client_id 
                                                        WHERE 
                                                            queue_details.service = 'Police' AND queue_details.`status` = 1 AND client.created_at >= CURRENT_DATE - INTERVAL 7 DAY
                                                        ORDER BY 
                                                            client.client_id DESC;                                                  ";
                                                break;
                                            case '1':
                                            ...
                                            case '12':
                                                $sql_select = "SELECT DISTINCT CONCAT(client.f_name, ' ' , client.l_name, ' ' ,client.suffix) AS Name, age.age_range AS Age, client.Gender, queue_details.created_at AS TimeIn, feedback.created_at AS TimeOut FROM client INNER JOIN queue_details
                                                                            ON client.client_id = queue_details.client_id INNER JOIN age ON  client.age_id = age.age_id INNER JOIN feedback ON client.client_id = feedback.client_id WHERE queue_details.service = 'Police' AND queue_details.`status` = 1 AND MONTH(client.created_at) = $filter ORDER BY client.client_id DESC;";
                                                break;
                                            case $filter:
                                                $sql_select = "SELECT DISTINCT 
                                                            CONCAT(client.f_name, ' ', client.l_name, ' ', IFNULL(client.suffix, '')) AS Name, 
